...
if (-not (Test-Path -Path $mqscFile)) {"Файл $mqscFile не существует"}
#Чтение содержимого файла
$mqsc = Get-Content $mqscFile
...
function printObj($obj) {
    switch ( $obj.type ) {
        QLOCAL {
            $file = $currentDir + "table\$qmname\qlocal.html"
        ...
        CHANNEL {
            $file = $currentDir + "table\$qmname\channel.html"
            "<tr>`n" +
            "<td rowspan='3'>" + $obj.NAME + "</td>`n" +
